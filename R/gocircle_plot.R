...
getGores <- function(x,
  ...
  df <- df %>%
    tidyr::drop_na(ID, ONTOLOGY, BgRatio, p.adjust, geneID) %>%
    dplyr::mutate(
  ...
  long_df <- df %>%
    dplyr::select(goterm, category, totalnumber, termnumber, p.adjust, rich_factor, geneID) %>%
  ...
  circlize::circos.genomicTrackPlotRegion(
    ...
    panel.fun = function(region, value, ...) {
      ...
      if (label_data[sector.index, "up_counts"] > 0) {
        ...
        circlize::circos.text(xlim, ylim, label_data[sector.index, "up_counts"], cex = 0.6, col = up_col, font = par("font"), niceFacing = FALSE)
      }

      if (label_data[sector.index, "down_counts"] > 0) {
        xlim <- (label_data[sector.index, "total_width"] + label_data[sector.index, "up_width"]) / 2
        circlize::circos.text(xlim, ylim, label_data[sector.index, "down_counts"], cex = 0.6, col = down_col, font = par("font"), niceFacing = FALSE)
      }
    }
  )
  
  ## fourth round
  plot_data <- x1 %>% dplyr::select(ID, gene_num.min, gene_num.max, RichFactor)
