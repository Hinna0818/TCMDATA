...
#' @importFrom dplyr count mutate group_by ungroup arrange select filter lead case_when
    ...
    knot.pos = 0.3)
{
  ...
  sankeyPlot <- ggplot(data = sankeyData,
    ...
    geom_text(stat = "stratum",
              data = function(x) filter(x, axis == "herb"),
    ...
    # molecule	
    geom_text(stat = "stratum",
              data = function(x) filter(x, axis == "molecule"),
              aes(label = ifelse(grepl("spacer_", as.character(after_stat(stratum))), "",
                                 as.character(after_stat(stratum)))),
              hjust = 0.5, nudge_x = 0, 
              size = font_size, family = plot_font, fontface = font_face) +
    ...
    # target
    geom_text(stat = "stratum",
              data = function(x) filter(x, axis == "target"),
              aes(label = yulab.utils::str_wrap(
                ifelse(grepl("spacer_", as.character(after_stat(stratum))), "",
                       as.character(after_stat(stratum))),50)),    
