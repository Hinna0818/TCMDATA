...
#'
#' @importFrom stats phyper na.omit
...
herb_ora_one <- function(genes, herb_targets, universe = NULL) {
  ...
  herb_targets <- unique(stats::na.omit(as.character(herb_targets)))

  # default universe: all TCM targets
  if (is.null(universe)) {
    universe <- unique(stats::na.omit(as.character(tcm_data$target)))
  }
  ...
  if (is.numeric(universe) && length(universe) == 1L) {
    ...
    bg <- unique(stats::na.omit(as.character(universe)))
