...
1. `search_disease()` and `search_gene_disease()` retrieve disease-gene associations from the [DisGeNET](https://www.disgenet.org/) dataset bundled in the [`DOSE`](https://bioconductor.org/packages/DOSE/) Bioconductor package.
2. `search_disease_efo()`, `get_disease_targets()`, and `query_disease_targets()` retrieve scored disease-target associations from the [Open Targets Platform](https://platform.opentargets.org/) GraphQL API.
...
```{r search-disease-demo, eval=FALSE, collapse=FALSE}
library(TCMDATA)
...
```{r search-disease-cui, eval=FALSE, collapse=FALSE}
## exact match by CUI
...
```{r search-gene-disease-demo, eval=FALSE, collapse=FALSE}
...
```{r search-gene-multi, eval=FALSE, collapse=FALSE}
...
### Open Targets API: disease → scored targets
...
1. Search the disease ontology identifier.
2. Retrieve associated targets by EFO/MONDO/Orphanet ID.
3. Keep the `score` column for downstream weighting.

```{r opentargets-api-demo, eval=FALSE, collapse=FALSE}
library(TCMDATA)
...
```{r opentargets-query-demo, eval=FALSE, collapse=FALSE}
...
```{r dn-otp-tbl-demo, eval=FALSE, collapse=FALSE}
data(dn_otp_tbl)
...
![](images/open_target_platform.png)
...
```{r upset-plot, collapse=FALSE, eval=FALSE}
gene_list <- list(
  DEGs = degs,
  GeneCards  = dn_gcds,
  OpenTargets = dn_otp
...
## PPI-weighted target ranking {#ppi-weighted-target-ranking}
...
1. Disease targets from different databases may have different levels of evidence.
2. A non-overlapping herb target may still be biologically relevant if it is close to the disease module in the human PPI network.
...
### Example: Lingzhi targets against DN disease module
...
```{r ppi-weighted-ranking-demo, eval=FALSE, collapse=FALSE}
library(TCMDATA)
...
```{r select-ppi-ranked-targets, eval=FALSE, collapse=FALSE}
